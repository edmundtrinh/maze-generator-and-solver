...
public class SearchEngine {
    ...
    public SearchEngine(String dataFolderName) {
        ...
        if (flag) { System.out.println("Done loading pages (" + (end / 1000.0) + " sec)"); }
        this.pages = this.extractWebpageSummaries(webpages);
        if (flag) { System.out.println("Done extracting"); }

        start = System.currentTimeMillis();
        if (flag) { System.out.println("Started TfIdfAnalyzer"); }
        this.tfIdfAnalyzer = new TfIdfAnalyzer(webpages);
        end = System.currentTimeMillis() - start;
        if (flag) { System.out.println("Finished TfIdfAnalyzer (" + (end / 1000.0) + " sec)"); }
        ...
        if (flag) { System.out.println("Done indexing (" + (end / 1000.0) + " sec)"); }
    }
