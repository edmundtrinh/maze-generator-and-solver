...
public class TestDoubleLinkedList extends BaseTest {
    ...
    public void testDeleteAtEndIsEfficient() {
        ...
        list.delete(cap - 1);
        assertEquals(cap - 2, list.get(list.size() - 1));
    }
    ...
    public void testDeleteNearEndIsEfficient() {
        ...
        assertEquals(cap, list.size());
        int temp = list.delete((2*cap)/3);
        assertEquals((2*cap)/3, temp);
        assertEquals(cap - 1, list.size());
        assertEquals(0, list.get(0));
        assertEquals(((2*cap)/3) - 2, list.get(((2*cap)/3) - 2));
        assertEquals(((2*cap)/3) - 1, list.get(((2*cap)/3) - 1));
        assertEquals(((2*cap)/3) + 1, list.get((2*cap)/3));
    }
