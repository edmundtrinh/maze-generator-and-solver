...
public class TestChainedHashDictionary extends TestDictionary {
    ...
    public void stressTestIterator() {
        int limit = 1000000;
        IDictionary<Integer, Integer> dict = this.newDictionary();
        ...
        System.out.println("Done inserting into ChainedHash & int[] (" + (end / 1000.0) + " sec)");

        start = System.currentTimeMillis();
        for (int i = 0; i < limit; i++) {
            assertEquals(i, test[i]);
        }
        end = System.currentTimeMillis() - start;
        System.out.println("Done iterating over int[] (" + (end / 1000.0) + " sec)");
        
        int j = 0;
        start = System.currentTimeMillis();
        for (KVPair<Integer, Integer> pair : dict) {
            assertEquals(j, pair.getKey());
            j++;
        }
        end = System.currentTimeMillis() - start;
        System.out.println("Done iterating over ChainedHashDict (" + (end / 1000.0) + " sec)");
    }
}
